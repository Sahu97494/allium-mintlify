...
description: NFT Trades events tagged with wash trading flags and filters.
---

The `nfts.trades_with_wash_trading_flags` table builds on top of the NFT trades model to detect wash trading activities on NFT marketplaces.&#x20;

## Approach

Wash trading can be inferred through onchain movement of NFTs and funding patterns across different buyers and sellers of NFTs.

In our model, we consider multiple factors as input factors, assign a score to each factor, and then compute a wash trading score based on the sum of these factors.

The output is then classified into different levels of wash trading ranging from `very low` to `very high`. The Wash Trading Flag is customizable, and users can query extra columns for a more granular view.
...
* `buyer_birth_funding_wallet`: address that first sent an ERC20/Native token to the buyer
* `seller_birth_funding_wallet`: address that first sent an ERC20/Native token to the seller
...
* `count_same_nft_trades_by_seller`: number of times the buyer traded the NFT(except for the first time) in a window of +-7 days since the given trade
* `seller_funded_buyer_recently`: true if the buyer sent ERC20s/Native tokens to the seller 24h before or after the trade
